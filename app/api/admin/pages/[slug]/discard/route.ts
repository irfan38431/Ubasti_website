...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    await db.update(pages).set({ draftBlocks: null, updatedAt: new Date(), updatedBy: adminId }).where(eq(pages.slug, slug));
    void audit({ actorUserId: adminId, action: "cms.page.discard", targetType: "page", targetId: slug });
    return NextResponse.json({ ok: true, blocks: row.blocks });
  } catch {
